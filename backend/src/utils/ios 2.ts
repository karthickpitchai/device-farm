      ...
      if (isSimulator) {
        ...
        try {
          ...
          if (cacheValid && cached) {
            ...
            const scale = Math.min(scaleX, scaleY); // Use minimum to account for aspect ratio

            logger.info(`Scale factor: ${scale.toFixed(2)}x (screenshot: ${imageSize.width}x${imageSize.height}, window: ${winWidth}x${winHeight})`);
          ...
          logger.info(`Scale: ${scale.toFixed(2)}x, scaled coords: (${scaledX.toFixed(0)}, ${scaledY.toFixed(0)})`);
          logger.info(`Tap: device coords (${x}, ${y}) -> scaled (${scaledX.toFixed(0)}, ${scaledY.toFixed(0)}) -> screen coords (${absX}, ${absY})`);

          // Step 4: Use AppleScript to click (more reliable than cliclick)
          const clickScript = `
          ...
          await execAsync(`osascript -e '${clickScript.replace(/'/g, "'\\''")}'`);

          logger.info(`iOS Simulator tap executed successfully`);
        } catch (tapError) {
          logger.error(`iOS Simulator tap failed: ${tapError}`);
          throw tapError;
        }
...
`;
    ...
    try {
      ...
      if (isSimulator) {
        ...
        try {
          ...
          await execAsync(`cliclick dd:${absStartX},${absStartY} w:${swipeDuration / 1000} du:${absEndX},${absEndY}`);

          logger.info(`iOS Simulator swipe executed successfully`);
        } catch (swipeError) {
          logger.error(`iOS Simulator swipe failed: ${swipeError}`);
          throw swipeError;
        }
  ...
  async inputText(deviceId: string, text: string): Promise<void> {
    try {
      ...
      if (isSimulator) {
        // For iOS simulators, we can set text to the pasteboard
        // This is a partial solution - the text will be available for manual paste
